...
function defaultState(){
  ...
  return{
    version:"4.4-r6",activeBranch:"mile",target:"jetblack",customTarget:"",
...
function parentBuildAssessment(name,b){
  var ownType=cfg.slots[b].target,ownBase=aptitudeOf(name,ownType),dirtBase=aptitudeOf(name,"ダート"),needDirt=starsNeeded(dirtBase,"D"),needA=starsNeeded(ownBase,"A"),needB=starsNeeded(ownBase,"B"),dirtSlots=slotCountForStars(needDirt),aSlots=slotCountForStars(needA),bSlots=slotCountForStars(needB),feasibleA=dirtSlots+aSlots<=6,feasibleB=dirtSlots+bSlots<=6,targetRank=feasibleA?"A":(feasibleB?"B":null),ownSlots=feasibleA?aSlots:(feasibleB?bSlots:99),used=ownSlots+dirtSlots;
  return{name:name,branch:b,ownType:ownType,ownBase:ownBase,dirtBase:dirtBase,needDirt:needDirt,needA:needA,needB:needB,dirtSlots:dirtSlots,ownSlots:ownSlots,usedSlots:used,feasibleA:feasibleA,feasibleB:feasibleB,targetRank:targetRank,feasible:!!targetRank};
}
function projectedParentAptitude(name,b,type,build){build=build||parentBuildAssessment(name,b);if(type===build.ownType&&build.feasible)return resultRank(aptitudeOf(name,type),build.ownSlots*3);if(type==="ダート"&&build.feasible)return resultRank(aptitudeOf(name,"ダート"),build.dirtSlots*3);return aptitudeOf(name,type)}
function pairCommonG1Potential(mileName,chaseName,mBuild,cBuild){var seen={};return cfg.races.filter(function(r){if(seen[r.name])return false;var mGround=projectedParentAptitude(mileName,"mile",r.ground,mBuild),mDist=projectedParentAptitude(mileName,"mile",r.distance,mBuild),cGround=projectedParentAptitude(chaseName,"chase",r.ground,cBuild),cDist=projectedParentAptitude(chaseName,"chase",r.distance,cBuild);var ok=raceReadyRanks(mGround,mDist,r)&&raceReadyRanks(cGround,cDist,r)&&!blockedReason(mileName,r)&&!blockedReason(chaseName,r);if(ok)seen[r.name]=1;return ok}).length}
function directPairPlans(){
  if(state.target!=="jetblack")return[];
  var names=Object.keys(DIRECT_PARENT_META).filter(function(n){return cfg.familyCharacters[n]&&(!state.onlyOwned||state.owned[n])}),out=[];
  names.forEach(function(m){names.forEach(function(c){if(m===c)return;var mb=parentBuildAssessment(m,"mile"),cb=parentBuildAssessment(c,"chase"),feasible=mb.feasible&&cb.feasible,mc=knownCompat("シンボリクリスエス",m)||0,cc=knownCompat("シンボリクリスエス",c)||0,pc=knownCompat(m,c)||0,compat=mc+cc+pc,common=feasible?pairCommonG1Potential(m,c,mb,cb):0,examM=inheritanceOwnedState(m)===false?0:(DIRECT_PARENT_META[m].examScore||0),examC=inheritanceOwnedState(c)===false?0:(DIRECT_PARENT_META[c].examScore||0),exam=examM+examC,quality=(mb.feasibleA?18:(mb.feasibleB?8:-80))+(cb.feasibleA?18:(cb.feasibleB?8:-80)),pressure=feasible?(mb.usedSlots+cb.usedSlots):30,finishRisk=(mb.targetRank==="B"?12:0)+(cb.targetRank==="B"?12:0),score=exam/20+compat*2+common*2+quality-pressure*1.5-finishRisk;
    if(!feasible)score-=1000;
    out.push({mile:m,chase:c,mileBuild:mb,chaseBuild:cb,examScore:exam,targetCompatMile:mc,targetCompatChase:cc,pairCompat:pc,compat:compat,commonG1:common,finishRisk:finishRisk,score:score,feasible:feasible,scoreBreakdown:{exam:Math.round(exam/20*10)/10,compat:compat*2,commonG1:common*2,buildQuality:quality,slotPressure:Math.round(-pressure*1.5*10)/10,finishRisk:-finishRisk}});
  })});
...
function requiredParentForBranch(b){if(state.target!=="jetblack")return"";var p=bestDirectParentPlan();return p?(b==="mile"?p.mile:p.chase):""}
...
function parentPlanStatus(b){var parent=selected(b,"parent");if(!parent)return null;var ownType=redType(b,"parent"),ownRank=projectedRank(b,"parent",ownType),dirtRank=projectedRank(b,"parent","ダート"),req=requiredParentForBranch(b),minOwn=parentOwnMinimumRank(b,parent),nameOk=!req||parent===req,ownOk=gv(ownRank)>=gv(minOwn),dirtOk=gv(dirtRank)>=gv("D");return{parent:parent,required:req,nameOk:nameOk,ownType:ownType,ownRank:ownRank,minOwn:minOwn,dirtRank:dirtRank,ownOk:ownOk,dirtOk:dirtOk,ok:nameOk&&ownOk&&dirtOk}}
...
function autoDesignAll(){ensureRedTypes();var plan=applyFinalRedPlan();rebuildBranchWithPlan("mile");rebuildBranchWithPlan("chase");/* 候補選択後にもう一度必須赤因子を確定し、その赤因子に合う曾祖父母を再選定 */["mile","chase"].forEach(function(b){planAncestorRedsForParent(b);["a1","a2","a3","a4"].forEach(function(s){state.branches[b].selection[s]=best(b,s)})});applyCommonParentRotation();var st=corePlanStatus(),bp=bestDirectParentPlan();save(st.ok?"技能試験向けに直親を再計算："+(bp?bp.mile+"＋"+bp.chase:"推奨ペア")+"。クリスエス開始時マイルA・追込B、両親ダートDを満たす設計です":"必須条件を満たせない箇所があります。所持縛り・候補を確認してください");return plan}
...
function backupFile(){var payload=clone(state);payload.backupMeta={version:"4.4-r6",createdAt:new Date().toISOString(),target:targetName(),skills:selectedSkillNames().length};return new File([JSON.stringify(payload,null,2)],"ウマ娘自動因子設計_v44r6_バックアップ.json",{type:"application/json"})}
