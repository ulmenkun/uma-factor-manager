...
function migrate(old){
  ...
  ["activeBranch","target","customTarget","initial","desired","onlyOwned","owned","scenarioMode","compatDirtFocus"].forEach(function(k){if(old[k]!==undefined)fresh[k]=old[k]});
...
function planAncestorRedsForParent(b){var parent=selected(b,"parent");if(!parent)return;var ownType=redType(b,"parent"),free=["a1","a2","a3","a4"],gpOwn=(redType(b,"gp1")===ownType?star(b,"gp1"):0)+(redType(b,"gp2")===ownType?star(b,"gp2"):0),gpDirt=(redType(b,"gp1")==="ダート"?star(b,"gp1"):0)+(redType(b,"gp2")==="ダート"?star(b,"gp2"):0),needDirt=state.compatDirtFocus!==false&&ownType!=="ダート",chosen=null;
...
function parentPlanStatus(b){var parent=selected(b,"parent");if(!parent)return null;var ownType=redType(b,"parent"),ownRank=projectedRank(b,"parent",ownType),dirtRank=projectedRank(b,"parent","ダート"),req=requiredParentForBranch(b),nameOk=!req||parent===req,ownOk=gv(ownRank)>=gv("A"),dirtOk=state.compatDirtFocus===false||gv(dirtRank)>=gv("D");return{parent:parent,required:req,nameOk:nameOk,ownType:ownType,ownRank:ownRank,dirtRank:dirtRank,ownOk:ownOk,dirtOk:dirtOk,ok:nameOk&&ownOk&&dirtOk}}
...
function grandparentUpstreamSummaryHtml(){
  if(state.compatDirtFocus===false)return"";
...
function reason(b,name){var c=cfg.familyCharacters[name],a=[],rt=activeSlot?redType(b,activeSlot):cfg.slots[b].target,rr=aptitudeOf(name,rt),need=starsNeeded(rr,"A");a.push("推奨赤因子「"+rt+"」：初期"+rr+(need===0?"でそのままA":"／Aまで"+(isFinite(need)?need+"★":"育成中継承が必要")));var d=(c.surface||{})["ダート"]||"G",dn=starsNeeded(d,"D");if(state.compatDirtFocus)a.push("ダート"+d+(dn===0?"でローテ可":"→Dに"+(isFinite(dn)?dn+"★":"追加継承が必要")));var cov=["マイル","中距離","長距離"].filter(function(k){return gv((c.distance||{})[k]||"G")>=gv("B")}).length;if(cov>=2)a.push("複数距離G1向き");return a.join("。")+"。"}
...
function bind(){
  ...
  var dirtFocus=document.getElementById("compatDirtFocus");if(dirtFocus){dirtFocus.checked=state.compatDirtFocus!==false;dirtFocus.addEventListener("change",function(){state.compatDirtFocus=this.checked;autoDesignAll();renderTree();renderSchedule()})}
