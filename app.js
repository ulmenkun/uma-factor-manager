...
function defaultState(){
  ...
  return{
    version:"3.9",activeBranch:"mile",target:"jetblack",customTarget:"",
    initial:clone(target.initial),desired:clone(target.desired),
    selectedSkills:defaultSelectedSkills(),onlyOwned:false,owned:{},
    branches:{
      mile:{selection:{},stars:{},skillFactors:{},hiddenSkills:{},raceFactors:{},raceListOverrides:{},races:{},compat:{}},
      chase:{selection:{},stars:{},skillFactors:{},hiddenSkills:{},raceFactors:{},raceListOverrides:{},races:{},compat:{}}
    },
...
function canRace(name,r){var c=cfg.familyCharacters[name];if(!c)return false;return gv(c.surface[r.ground]||"G")>=gv("B")&&gv(c.distance[r.distance]||"G")>=gv("B")}
function targetCanRace(r){return gv(state.initial[r.ground]||"G")>=gv("B")&&gv(state.initial[r.distance]||"G")>=gv("B")}
function coverage(name){var c=cfg.familyCharacters[name];if(!c)return 0;var s=0;["マイル","中距離","長距離"].forEach(function(k){var v=gv(c.distance[k]);s+=v>=gv("A")?8:v>=gv("B")?6:v>=gv("C")?3:0});if(gv(c.surface["ダート"])>=gv("B"))s+=8;return s}
function candidateScore(b,slot,name){var c=cfg.familyCharacters[name],def=cfg.slots[b].nodes[slot];if(!c)return-999;if(state.onlyOwned&&!state.owned[name])return-999;var i=def.prefs.indexOf(name),score=i>=0?100-i*9:45;score+=coverage(name);if(b==="chase"&&gv(c.style["追込"])>=gv("A"))score+=16;if(b==="mile"&&gv(c.distance["マイル"])>=gv("A"))score+=12;if(state.owned[name])score+=8;if(others(b,slot).indexOf(name)>=0)score-=45;return score}
...
function rebuild(b,start){var order=["parent","gp1","gp2","a1","a2","a3","a4"],idx=Math.max(0,order.indexOf(start||"parent"));for(var i=idx;i<order.length;i++){var s=order[i];state.branches[b].selection[s]=best(b,s);if(state.branches[b].stars[s]===undefined)state.branches[b].stars[s]=2}save()}
...
function nodeHtml(b,slot,kind){var n=selected(b,slot)||"未選択",count=nodeAssignedCount(b,slot),base=baseCompatibility(b,slot),g1=stageG1Bonus(b,slot),total=base+g1;return'<button type="button" class="tree-node '+kind+'" data-slot="'+slot+'"><span class="node-role">'+cfg.slots[b].nodes[slot].role+'</span><span class="node-name">'+esc(n)+'</span><span class="node-meta"><span class="node-star">'+star(b,slot)+'★</span><span class="node-score '+(count?"best":"")+'">因子'+count+'</span></span><span class="node-compat '+(total>=70?"high":total>=45?"mid":"")+'">相'+base+'＋G1 '+g1+'＝'+total+'</span></button>'}
function renderTree(){var b=activeBranch,finalBonus=finalG1Bonus(),finalBase=finalBaseCompatibility(),finalTotal=finalCompatibilityTotal();document.getElementById("familyTree").innerHTML='<div class="tree-target"><button type="button" class="tree-node target" data-target-node><span class="node-role">育成ウマ娘</span><span class="node-name">'+esc(targetName())+'</span><span class="node-meta"><span class="node-g1">両親共通G1 +'+finalBonus+'</span></span><span class="node-compat '+(finalTotal>=100?"high":finalTotal>=70?"mid":"")+'">相'+finalBase+'＋G1 '+finalBonus+'＝'+finalTotal+'</span></button></div><div class="tree-level parent">'+nodeHtml(b,"parent","parent")+'</div><div class="tree-level grandparents">'+nodeHtml(b,"gp1","grandparent")+nodeHtml(b,"gp2","grandparent")+'</div><div class="tree-level ancestors">'+nodeHtml(b,"a1","ancestor")+nodeHtml(b,"a2","ancestor")+nodeHtml(b,"a3","ancestor")+nodeHtml(b,"a4","ancestor")+'</div>';document.querySelectorAll(".tree-node[data-slot]").forEach(function(e){e.addEventListener("click",function(){openNode(e.dataset.slot)})});var t=branchTotal(b);document.getElementById("branchStarTotal").textContent=t+"★";document.getElementById("geneCondition").textContent=t>=12?"条件達成":"あと"+(12-t)+"★";document.getElementById("finalAptitude").textContent=resultRank(cfg.slots[b].base,finalStars(b));var fs=document.getElementById("finalG1Summary");if(fs)fs.innerHTML='<span>両親のトータル相性</span><strong>'+finalTotal+'</strong><small>基礎相性 '+finalBase+' ＋ 共通G1 '+finalBonus+'</small>';var gs=document.getElementById("g1StageSummary");if(gs)gs.innerHTML=["parent","gp1","gp2"].map(function(s){var base=baseCompatibility(b,s),g=stageG1Bonus(b,s),v=base+g;return'<div class="stage-card '+bonusClass(g)+'"><small>'+cfg.slots[b].nodes[s].role+' '+esc(selected(b,s))+'</small><strong>合計 '+v+'</strong><span class="stage-breakdown">基礎相性 '+base+' ＋ 父母共通G1 '+g+'</span></div>'}).join("");renderFinalInheritanceSummary()}
...
function commonCount(b,slot){var parent=slot==="parent"?null:(slot==="gp1"||slot==="gp2"?"parent":(slot==="a1"||slot==="a2"?"gp1":"gp2")),name=selected(b,slot),pn=parent?selected(b,parent):null;return cfg.races.filter(function(r){return name&&canRace(name,r)&&(parent?pn&&canRace(pn,r):targetCanRace(r))}).length}
...
function backupFile(){var payload=clone(state);payload.backupMeta={version:"3.9",createdAt:new Date().toISOString(),target:targetName(),skills:selectedSkillNames().length};return new File([JSON.stringify(payload,null,2)],"ウマ娘自動因子設計_v39_バックアップ.json",{type:"application/json"})}
